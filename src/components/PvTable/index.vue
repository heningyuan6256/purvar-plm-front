<template>
  <div>
      <vxe-toolbar v-if="tool" perfect :custom="filter">
        <!-- <el-button>导出</el-button> -->
        <template v-slot:buttons>
            <vxe-input v-if="search" v-model="searchVal" class="query" size="small" placeholder="搜索"
        ...
        <template v-slot:tools>
          <span v-if="btnFlag" class="btn-sty">
            <el-button v-for="(item, index) in buttonColumn"
            :key="index"
            :type="item.type"
            size="mini"
            @click="dealEvent(item.name)">{{ item.label }}</el-button>
          </span>
            <el-button v-if="exportVisible" size="mini" type="primary" @click="exportData">导出</el-button>
            <el-button v-if="printVisible" size="mini" type="info" @click="printEvent" class="pd-10">打印</el-button>
        </template>
      </vxe-toolbar>
      <vxe-table
      id="table"
